# Laragon Linux — Static PHP Binaries (Phase 2, Version slice 1b) Design Spec

**Date:** 2026-06-24
**Status:** Design approved, pending spec review
**Goal:** Source every PHP version from prebuilt **static** php-fpm binaries (dl.static-php.dev, `bulk` preset) downloaded into `~/laragon/bin`, replacing the apt/ondrej install path entirely. This makes PHP install/switch work on **any** distro (including Ubuntu 26.04 "resolute", where ondrej has no packages) and needs **no root/pkexec** (everything lives under the user-owned `~/laragon`).

This supersedes the apt/ondrej install added in the previous slice (PHP Version Management), which is removed here. Detection, the active-version switch (`Orchestrator::replace_php_version`), and the Settings UI from that slice are **kept unchanged** — they already work for any `php-fpm<minor>` found in `~/laragon/bin`.

---

## 1. Context & current state

PHP version is global (`Config.php_version`). `PhpFpmService` writes `etc/php/<ver>/php-fpm.conf`, listens on the **version-independent** socket `tmp/php-fpm.sock`, and spawns `php-fpm<version>`. Crucially, the orchestrator resolves a service's program name through `~/laragon/bin` before PATH (`orchestrator.rs:77`, via `bin::resolve_or_name`) — the same mechanism that runs the downloaded `mailpit` binary. So **a binary placed at `~/laragon/bin/php-fpm<minor>` is detected and spawned with no further wiring**:
- `bin::list_php_fpm_versions(&[paths.bin()])` already lists it,
- `php_versions(...)` marks it installed,
- `Orchestrator::replace_php_version(v)` spawns `php-fpm<v>` resolved from `~/laragon/bin`.
...
- The `bulk` build has **65 modules** including the full Laragon-parity set and more: `intl`, `mysqli`, `imagick`, `sodium`, `xsl`, `pdo_mysql`, `gd`, `redis`, `curl`, `mbstring`, `zip`, `exif`, `bcmath`, `gmp`, `soap`, `opcache`, `sqlite3`, `pdo_pgsql`, … (the `common` preset lacks `intl`/`mysqli`/`imagick`/`sodium`/`xsl`, so `bulk` is chosen).
- Directory JSON: `…/bulk/?format=json` returns an array of `{ name, size, last_modified, … }`, used to resolve a minor to its newest patch.

## 2. Approach (chosen: static-only, `bulk` preset, into `~/laragon/bin`)

Add a `php_static` core module that resolves the newest patch for a requested minor from the `bulk` directory JSON, downloads the fpm tarball, extracts the `php-fpm` binary, and installs it as `~/laragon/bin/php-fpm<minor>`. Wire it into both the version-manager Install command and the Setup wizard. Remove the apt/ondrej PHP path.
...
- `pub fn install_php_static(paths: &LaragonPaths, version: &str, downloader: &dyn Downloader, runner: &dyn CommandRunner) -> Result<(), PhpStaticError>`:
  ...
  6. move/rename `tmp/php-fpm` → `~/laragon/bin/php-fpm<version>`; ensure mode `0o755` (`std::fs::set_permissions`). `bin/` is created if missing.
...
- `run_setup` gains a `runner: &dyn CommandRunner` parameter (so it can call `install_php_static`). Both callers (`commands.rs::run_setup_cmd`, `laragonctl`) pass `&RealCommandRunner`.
...
- **No privilege for PHP**: static install writes only under `~/laragon` (`tmp`, `bin`) — no pkexec prompt for installing/switching PHP. (Setup still uses pkexec for the apt core stack + setcap + hosts.)
...
- `install_php_static` with module-local fakes: a downloader that writes a provided JSON for the index URL and dummy bytes for the tarball URL, and a CommandRunner that, on the `tar` call, creates a `php-fpm` file in the dest dir — assert (a) the index URL then the resolved tarball URL were fetched in order, (b) `tar -xzf … -C … php-fpm` was invoked, (c) `~/laragon/bin/php-fpm8.4` exists and is mode `0o755`; an unavailable version yields `PhpStaticError::Unavailable`.
