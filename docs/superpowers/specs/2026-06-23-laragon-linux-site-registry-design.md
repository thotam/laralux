# Laragon Linux — Site Registry / Add Existing Folder (Phase 2, Slice 2) Design Spec

**Date:** 2026-06-23
**Status:** Design approved, pending spec review
**Goal:** Let a site point at a folder **outside** `~/laragon/www/`, persisted in `~/laragon/sites.toml`, reachable at `https://<name>.dev` exactly like an auto-discovered site; and let users unlink registered sites from the UI.
...
Today a "site" is purely auto-discovered: `core::sites::scan_sites` lists immediate subdirectories of `~/laragon/www/`, each becoming `Site { name, root, hostname }` with `document_root()` preferring `<root>/public`. `core::sync::sync_sites` calls `scan_sites`, issues a mkcert cert per site, writes a per-site HTTPS vhost, and updates the managed `/etc/hosts` block. The GUI `stack_start_all` and `create_site` both run `sync_sites` before serving.
...
- `LaragonPaths::sites_file()` → `root.join("sites.toml")` (new accessor).
...
- `pub fn list_all_sites(paths: &LaragonPaths, tld: &str) -> std::io::Result<(Vec<Site>, Vec<String>)>`:
