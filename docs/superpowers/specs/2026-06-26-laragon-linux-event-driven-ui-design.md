# Laragon Linux — Event-Driven Realtime UI Design
...
2. **A filesystem watcher** (`notify` crate) on `~/laragon/www` + `~/laragon/sites.toml` that emits `sites-changed` (debounced) when a site folder/registry changes on disk — covering external edits in realtime.
...
```rust
let handle = app.handle().clone();
let paths = handle.state::<AppState>().paths.clone(); // LaragonPaths is Clone
let www = paths.www();
...
- `notify`/monitor are runtime/threaded and **verified live** (start a service then `kill` its process → the UI flips to Crashed within ~1 s without interaction; create a folder in `~/laragon/www` → it appears within ~300 ms). No unit test for the thread/watcher wiring (I/O + timing).
...
- Core is unchanged (zero new core code); existing `cargo test -p laragon-core` stays green; `cargo build -p laragon-desktop` must compile with the new `notify` dep.

## 6. Out of scope (backlog)

- A filesystem watch on `~/laragon/bin` for realtime component-presence changes (rare; component refresh stays command-driven).
