...
- `pub fn apt_packages_for_php(version: &str) -> Vec<String>` → the **Laralux-parity** baseline, version-pinned (in this order):
...
- `apt_packages_for_php("8.3")` == the 13 Laralux-parity `php8.3-*` packages in the specified order (asserts `php8.3-fpm` first and that `php8.3-gd`, `php8.3-imagick`, `php8.3-redis`, `php8.3-xsl`, `php8.3-zip`, `php8.3-sqlite3` are present).
