# Laragon Linux — Design Spec

**Ngày:** 2026-06-18
**Trạng thái:** Đã duyệt thiết kế, chờ review spec
**Mục tiêu:** Xây dựng app quản lý môi trường dev cho Linux tương tự Laragon (Windows), bám theo stack thực tế người dùng đang dùng: Nginx + PHP-FPM + MariaDB + Redis + Node + Mailpit, pretty URL `*.dev`, chủ yếu phục vụ project Laravel.

---

## 1. Bối cảnh

Laragon (Windows, viết bằng Delphi) là app GUI chạy ở system tray, tự điều phối service (không dùng Windows services), tự sinh virtual host + pretty URL (`app.test`), tạo project 1-click (Laravel/WordPress), cấp SSL local, mở terminal/DB client. Triết lý: nhẹ (<6MB binary, ~4–10MB RAM), portable, isolated.

Người dùng đã chuyển sang **Ubuntu 26.04 LTS** và muốn một app tương đương cho Linux. Stack thực tế (trích từ `laragon.ini` Windows):
...
| Điều phối service | **Phương án A** — app tự spawn/quản lý process con; config + data riêng trong `~/laragon/`, không đụng `/etc` hệ thống |
| Giao diện | **GUI desktop + system tray** |
| Framework | **Tauri** (Rust backend + frontend web) — nhẹ, hợp triết lý Laragon |
| Quyền hệ thống | **polkit/sudo khi cần** + `setcap` cho nginx để chạy hằng ngày không cần mật khẩu |
| Pretty URL | `*.dev` (cấu hình được trong `laragon.toml`, mặc định `dev`) |
| SSL | **Auto mkcert** (bắt buộc, vì `.dev` preload HSTS → buộc HTTPS) — nằm trong MVP |
| Phạm vi | Full parity Laragon, chia 3 phase; MVP dùng được hằng ngày |
...
└─────────────────────────────────┼────────────┘
   ...
   (port 80/443, config + data trong ~/laragon, KHÔNG đụng /etc)
```
...
~/laragon/
  ...
  laragon.toml        # cấu hình app (thay laragon.ini)
```
...
- **nginx:** `nginx -p ~/laragon/etc/nginx -c .../nginx.conf`; mỗi site 1 file trong `etc/nginx/sites/`.
- **php-fpm:** pool riêng, socket trong `tmp/`, chọn version qua binary `/usr/sbin/php-fpm<ver>`.
- **mariadb:** `--defaults-file=~/laragon/etc/mariadb/my.cnf --datadir=~/laragon/data/mariadb`; lần đầu tự `mariadb-install-db`.
...
- TLD cấu hình trong `laragon.toml` (mặc định `dev`).
...
- Mọi lỗi: log chi tiết trong `~/laragon/log/` + thông điệp ngắn trên UI.
