...
- Remove the `allow_mariadb_apparmor()` call (the tarball `mariadbd` lives under `~/laralux`, outside Ubuntu's AppArmor profile for `/usr/sbin/mariadbd`).
...
- **datadir init:** first start (`needs_init`) runs the bundled `mariadb-install-db` with `--basedir`/`--datadir`; the existing `~/laralux/data/mariadb` datadir + `my.cnf` are reused.
- **No new root:** mariadb is a user-space download under `~/laralux`; no AppArmor profile applies (so `allow_mariadb_apparmor` is unnecessary and removed). The remaining privileged steps are unchanged (hosts, setcap nginx, resolved drop-in, mkcert CA).
