# Laragon Linux — No-apt Spec 1 (nginx + redis/Valkey + mkcert) into the versioned layout

**Date:** 2026-06-26
**Status:** Design (goal-directed); supersedes the 2026-06-25 Spec-1 draft (which predated the versioned-binary layout).
**Goal:** Install **nginx**, **redis (Valkey)**, and **mkcert** as downloaded binaries into the versioned layout (`~/laragon/bin/<tool>/<version>/` + `current` symlink), removing them from `apt`. Composer/PHP/mailpit/coredns are already downloaded. Only MariaDB remains on apt afterward (Spec 2).
...
- **Self-contained:** after setup, `~/laragon/bin` holds `nginx/<ver>/nginx`, `redis/<ver>/{redis-server,redis-cli}`, `mkcert/<ver>/mkcert` alongside php/mailpit/coredns/composer — no distro packages for the stack except MariaDB (Spec 2).
