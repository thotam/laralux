...
- `main.rs` tray `"start_all"` handler: call `laralux_core::ensure_nginx_bind_cap(&state.paths, &laralux_core::PkexecPrivileged)` before `orch.start_all()`. The pkexec prompt (only when missing) briefly blocks the tray click — acceptable for a one-time, user-initiated action.
