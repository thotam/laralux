...
**Tech Stack:** Rust (laralux-core, zero Tauri deps), Tauri 2.
...
- Run core tests with `cargo test -p laralux-core`; build with `cargo build -p laralux-desktop`. If `cargo` isn't on PATH use `$HOME/.cargo/bin/cargo`.
...
**Interfaces:**
- Consumes: `resolve_bin`, `crate::paths::LaraluxPaths`, `crate::privileged::Privileged`.
- Produces:
...
Run: `cargo test -p laralux-core bin` then `cargo build -p laralux-core`
Expected: PASS — the parser test plus existing bin tests; crate builds (no unused-import warnings — `LaraluxPaths`/`Privileged` are used by `ensure_nginx_bind_cap`).
...
**Interfaces:**
- Consumes: `laralux_core::ensure_nginx_bind_cap`, `PkexecPrivileged`, `ServiceKind`.
- Produces: nginx capability is ensured before each Start path runs.

- [ ] **Step 1: Import the helper in commands.rs**

In `src-tauri/src/commands.rs`, add `ensure_nginx_bind_cap` to the `use laralux_core::{...}` imports (alongside the other core imports). `PkexecPrivileged` and `ServiceKind` are already imported.
...
```rust
                    "start_all" => {
                        if let Some(state) = app.try_state::<AppState>() {
                            laralux_core::ensure_nginx_bind_cap(
                                &state.paths,
                                &laralux_core::PkexecPrivileged,
                            );
                            if let Ok(mut orch) = state.orch.lock() {
                                let _ = orch.start_all();
                            }
...
**3. Type consistency:**
- `ensure_nginx_bind_cap(&LaraluxPaths, &dyn Privileged)` (Task 1) called identically from `stack_start_all`, `service_start`, and the tray handler (Task 2) with `&state.paths` + `&PkexecPrivileged`. ✓
