...
**Goal:** Restructure `~/laragon/bin` into a per-tool, per-version layout (`bin/<tool>/<version>/<binary>`) with a config-driven `current` symlink per tool, so versions are managed like Laragon for Windows.
...
**Tech Stack:** Rust (laragon-core, zero Tauri deps), Tauri 2, serde/toml, unix symlinks.

## Global Constraints

- `core` keeps **zero Tauri deps**. Commit messages MUST NOT contain a `Co-Authored-By` trailer. TDD: failing test first.
- Layout: `~/laragon/bin/<tool>/<version>/<binary…>`; active pointer `~/laragon/bin/<tool>/current → <version>` (relative symlink, target = the bare version string).
...
- Run core tests `cargo test -p laragon-core`; build `cargo build -p laragon-desktop && cargo build -p laragonctl`. If `cargo` isn't on PATH use `$HOME/.cargo/bin/cargo`.
...
**Interfaces produced:**
- `LaragonPaths::tool_dir(&self, tool: &str) -> PathBuf`, `version_dir(&self, tool, version) -> PathBuf`, `current_link(&self, tool) -> PathBuf`
...
- [ ] **Step 2: Run** — `cargo test -p laragon-core layout` → FAIL (functions/accessors missing).
...
- [ ] **Step 6: Run** — `cargo test -p laragon-core layout` → PASS; `cargo build -p laragon-desktop && cargo build -p laragonctl` clean.
...
- [ ] **Step 2: Run** — `cargo test -p laragon-core config` → FAIL.
...
- [ ] **Step 2: Run** — `cargo test -p laragon-core layout` → FAIL.
...
- [ ] **Step 5: Run** — `cargo test -p laragon-core layout` → PASS; build both crates clean.
...
- [ ] **Step 4: Run** — `cargo test -p laragon-core` → PASS (orchestrator/bin tests don't depend on real bin resolution); `cargo build -p laragon-desktop && cargo build -p laragonctl` clean.
...
fn download_static_php(
    paths: &LaragonPaths, version: &str, arch: &str, sapi: &str, member: &str,
...
pub fn install_php_static(
    paths: &LaragonPaths, requested: &str, downloader: &dyn Downloader, runner: &dyn CommandRunner,
) -> Result<String, PhpStaticError> {
...
- [ ] **Step 4: Run** — `cargo test -p laragon-core php_static` and `cargo test -p laragon-core` → PASS; `cargo build -p laragon-desktop && cargo build -p laragonctl` clean.
...
- [ ] **Step 2: Run** — `cargo test -p laragon-core php_versions` → FAIL (`installed_minors` missing).
...
- [ ] **Step 5: Update the remaining callers**:
  ...
  - `src-tauri/src/commands.rs`: remove `list_php_fpm_versions` from the `laragon_core` import list. The `set_php_version` guard at ~line 419 (`if !list_php_fpm_versions(&[state.paths.bin()]).contains(&version)`) changes to check installed minors:
```rust
...
- [ ] **Step 6: Run** — `cargo test -p laragon-core` → PASS; `cargo build -p laragon-desktop && cargo build -p laragonctl` clean.
...
pub fn ensure_active_php_cli(
    paths: &LaragonPaths, version: &str, downloader: &dyn Downloader, runner: &dyn CommandRunner,
...
- [ ] **Step 3: Run** — `cargo test -p laragon-core` → PASS (the coredns unit test only covers pure fns; the layout change is verified live). `cargo build -p laragon-desktop && cargo build -p laragonctl` clean.
...
- [ ] **Step 5: Manual verification (live)** — delete the old flat `~/laragon/bin` contents, run the desktop app's setup, and confirm: `~/laragon/bin/php/<ver>/{php,php-fpm}`, `bin/php/current → <ver>`, `bin/composer/<ver>/composer.phar`+wrapper, `bin/mailpit/<ver>/mailpit`, `bin/coredns/1.14.4/coredns`, each with a `current` symlink; the stack starts; `php -v` in a new terminal (with shell integration on) shows the active version; switching PHP repoints `bin/php/current`. (nginx/redis still come from apt until Spec 1.)
