...
**Goal:** Let a site point at a folder outside `~/laragon/www/` (persisted in `~/laragon/sites.toml`), reachable at `https://<name>.dev` like any scanned site, addable via a native folder picker, and unlinkable from the UI.
...
**Tech Stack:** Rust (laragon-core, no Tauri deps), Tauri 2 (`tauri-plugin-dialog`), vanilla JS frontend (`dist/`, `withGlobalTauri`).

## Global Constraints

- `core` keeps **zero Tauri deps**. The registry/merge logic lives in `laragon-core` behind plain functions; only `commands.rs`/`main.rs` touch Tauri.
...
- Registry file path: `~/laragon/sites.toml`. New site source values serialize as exactly `"Scanned"` / `"Linked"`.
- Run all core tests with `cargo test -p laragon-core`; build the app with `cargo build -p laragon-desktop`. If `cargo` is not on PATH, use `$HOME/.cargo/bin/cargo`.

---

### Task 1: `site_registry` module + `LaragonPaths::sites_file()`
...
**Interfaces:**
- Consumes: `crate::paths::LaragonPaths`, `crate::scaffold::validate_site_name`.
- Produces:
  - `LaragonPaths::sites_file(&self) -> PathBuf` = `root.join("sites.toml")`.
...
**Interfaces:**
- Consumes: `scan_sites`, `SiteRegistry::load`, `LaragonPaths::sites_file`.
- Produces:
  - `enum SiteSource { Scanned, Linked }` (serde, `Copy`).
  - `Site` gains `pub source: SiteSource`.
  - `pub fn list_all_sites(paths: &LaragonPaths, tld: &str) -> std::io::Result<(Vec<Site>, Vec<String>)>` — merged + sorted sites and human-readable warnings.
...
In `src-tauri/src/commands.rs`, update the first `use laragon_core::{...}` block to also import `list_all_sites`, `SiteRegistry`, and `SiteSource`:
...
- `list_all_sites(&LaragonPaths, &str) -> io::Result<(Vec<Site>, Vec<String>)>` defined in Task 2, consumed in Tasks 3/4. ✓
