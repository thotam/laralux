...
**Tech Stack:** Rust (laralux-core, zero Tauri deps), Tauri 2, vanilla JS; CoreDNS static binary (GitHub release), mkcert.
...
- CoreDNS: downloaded static binary into `~/laralux/bin/coredns` (no apt, no root for the binary), run on `127.0.0.1:5353`; only the systemd-resolved drop-in needs pkexec; runs only while wildcard domains exist; reversible.
...
- systemd-resolved drop-in: `/etc/systemd/resolved.conf.d/laralux.conf` with `[Resolve]\nDNS=127.0.0.1:5353\nDomains=~<base> …`.
- Run core tests `cargo test -p laralux-core`; build `cargo build -p laralux-desktop && cargo build -p laraluxctl`. If `cargo`/`node` aren't on PATH use `$HOME/.cargo/bin/cargo` / `$HOME/.nvm/versions/node/v24.16.0/bin/node`.
...
Run: `cargo test -p laralux-core site_registry` (PASS). Then:
...
- [ ] **Step 2: Run tests** — `cargo test -p laralux-core sites` → FAIL (no `domains` field).
...
- [ ] **Step 4: Run tests** — `cargo test -p laralux-core sites` (PASS, plus existing tests updated for the new field where they construct `Site` directly — update any direct `Site{...}` in sites.rs tests to include `domains`).
- [ ] **Step 5: Commit**
...
- [ ] **Step 2: Run tests** — `cargo test -p laralux-core ssl` → FAIL.
...
- [ ] **Step 4: Run tests** — `cargo test -p laralux-core ssl sync` (PASS; update sync tests that read `issuer.requested()` to the new `(String, Vec<String>)` shape).
- [ ] **Step 5: Commit**
...
**Files:** Modify `core/src/sync.rs`, `laraluxctl/src/main.rs`.
...
- [ ] **Step 2: Run** — `cargo test -p laralux-core sync` → FAIL (return type / fields).
...
In `laraluxctl/src/main.rs`, change the `sync_sites` match arm from `Ok(sites) => println!("Synced {} site(s).", sites.0.len())` to `Ok(out) => println!("Synced {} site(s).", out.sites.len())`.

(The Tauri `commands.rs` calls all use `let _ = sync_sites(...)`, so they still compile; Task 8 binds the outcome where needed.)

- [ ] **Step 4: Run tests + build** — `cargo test -p laralux-core` then `cargo build -p laraluxctl && cargo build -p laralux-desktop` (PASS; update the other sync tests to read `out.sites`/`out.warnings`).
- [ ] **Step 5: Commit**

```bash
git add core/src/sync.rs laraluxctl/src/main.rs
...
- [ ] **Step 2: Run** — `cargo test -p laralux-core privileged` → FAIL.
...
Run: `cargo test -p laralux-core privileged` (PASS).
...
- [ ] **Step 2: Run** — `cargo test -p laralux-core coredns` → FAIL.
...
/// Download the static CoreDNS binary into ~/laralux/bin (no apt/root) if missing.
...
Run: `cargo test -p laralux-core coredns` (PASS).
...
- [ ] **Step 2: Run** — `cargo test -p laralux-core orchestrator` → FAIL.
...
- [ ] **Step 4: Run tests** — `cargo test -p laralux-core` (PASS; if `ServiceKind` is matched exhaustively anywhere, add the `Coredns` arm). Build `cargo build -p laralux-desktop`.
- [ ] **Step 5: Commit**
...
- [ ] **Step 6: Build** — `cargo build -p laralux-desktop` (clean).
- [ ] **Step 7: Commit**
...
**3. Type consistency:** `ensure_cert(basename, names)` (T3) used by sync (T4) and FakeCertIssuer; `SyncOutcome{sites,warnings,wildcard_bases}` (T4) consumed by laraluxctl + commands (T8); `set_coredns(Vec<String>)` (T7) called by `apply_wildcard_dns` (T8); `ensure_coredns`/`resolved_dropin` (T6) used in T8; `write/remove_resolved_dropin` (T5) used in T8; `validate_domain`/`set_domains`/`domains_for` (T1) used in T2/T8; IPC `set_site_domains({name,domains})` matches the JS invoke (T9). Sequencing keeps each task compiling (commands' other `sync_sites` calls stay `let _`). ✓
