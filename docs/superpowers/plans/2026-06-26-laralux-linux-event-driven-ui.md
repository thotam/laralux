...
**Architecture:** Two detached backend threads spawned in `main.rs` `.setup`: one that calls `orch.refresh()`+`snapshot()` every ~1 s and emits `services-changed` only when the snapshot changes; one `notify` watcher on `~/laralux/www` + `sites.toml` that emits `sites-changed` (debounced). The frontend drops `setInterval(refresh,2000)`, keeps one startup `refresh()`, and updates from the events.

**Tech Stack:** Tauri 2 (Emitter/Manager + events), `notify` crate, vanilla JS. `laralux-core` is NOT touched (zero new core code).
...
- Commits MUST NOT contain a `Co-Authored-By` trailer. Build: `cargo build -p laralux-desktop`; `cargo test -p laralux-core` stays green; `node --check dist/app.js`. cargo fallback `$HOME/.cargo/bin/cargo`; node fallback `$HOME/.nvm/versions/node/v24.16.0/bin/node`.
...
```
(`laralux_core::ServiceStatus` is re-exported; `AppState` is imported via `use commands::{build_state, AppState};`. `app.handle()` returns the `AppHandle`; `.clone()` it for the thread.)

- [ ] **Step 2: Build** — `cargo build -p laralux-desktop` clean; `cargo test -p laralux-core` still green (unchanged).
...
- [ ] **Step 1: Add the dependency.** In `src-tauri/Cargo.toml` `[dependencies]`, add `notify = "6"` (use the latest 6.x that resolves; if 6 fails to resolve, use the current major that `cargo` accepts and note it). Run `cargo build -p laralux-desktop` to fetch/lock it.
...
```
(`LaraluxPaths` derives `Clone`; `paths.www()` and `paths.sites_file()` exist. The closure captures `handle` + `last` by move.)

- [ ] **Step 3: Build** — `cargo build -p laralux-desktop` clean.
...
- [ ] **Step 3: Manual verification (live)** — `cargo run -p laralux-desktop`: (a) start a service, then `kill` its OS process → the row flips to Crashed within ~1 s with no interaction; (b) `mkdir ~/laralux/www/<name>` → the site appears within ~300 ms; (c) idle for a while → no perpetual re-render (the UI is static when nothing changes); (d) scroll the page during a change → position is preserved.
