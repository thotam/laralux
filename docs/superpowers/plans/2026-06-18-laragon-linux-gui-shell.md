# Laragon Linux — Plan 3a: Tauri GUI Shell, Tray & Dashboard Implementation Plan
...
**Architecture:** A new `src-tauri` crate (`laragon-desktop`) joins the Cargo workspace. It holds the `laragon_core::Orchestrator` in Tauri managed state behind a `Mutex`, exposes `#[tauri::command]` IPC functions the frontend calls, draws a tray menu, and stops the stack on exit. A static HTML/JS frontend (`dist/`) uses the `window.__TAURI__` global (no JS bundler). `core` gains serde derives and a `snapshot()` so service state crosses the IPC boundary.

**Tech Stack:** Rust + Tauri 2 (`tray-icon`, `image-png` features), `serde`/`serde_json`, vanilla HTML/JS frontend, reuses `laragon_core` (Plan 1 + 2).

## Global Constraints

- App identifier: `com.laragon.linux`. Product/window title: `Laragon Linux`. Window label: `main`.
...
**Interfaces:**
- Produces: a buildable `laragon-desktop` binary that opens one empty window. Later tasks add state, commands, tray.
...
```
If these are absent, `cargo build -p laragon-desktop` fails with a `webkit2gtk-4.1` / `glib` pkg-config error; that means the prerequisite has not been run yet — report BLOCKED with the error so the human can install them.
...
```toml
members = ["core", "laragonctl", "src-tauri"]
```
...
```json
{
  ...
  "productName": "Laragon Linux",
  "version": "0.1.0",
  "identifier": "com.laragon.linux",
  "build": {
    "frontendDist": "../dist"
  },
  "app": {
...
`cargo run -p laragon-desktop` should open a 900×600 window titled "Laragon Linux". Note in the report that the live window check is a human step.
...
**Interfaces:**
- Consumes: `laragon_core::{Config, LaragonPaths, Orchestrator, RealSpawner, build_services, ServiceKind, ServiceStatus, scan_sites, Site}`.
- Produces:
  - `struct AppState { orch: std::sync::Mutex<Orchestrator>, paths: LaragonPaths, tld: String }`
...
```rust
use laragon_core::{
    build_services, scan_sites, Config, LaragonPaths, Orchestrator, RealSpawner, ServiceKind,
    ServiceStatus, Site,
};
use std::sync::Mutex;
...
`cargo run -p laragon-desktop`: a tray icon appears; its menu shows Start All / Stop All / Dashboard / Quit; `Quit` closes the app. Note this as a human verification step in the report.
...
- **Correctly deferred to Plan 3b:** setup wizard, apt install of the stack, mkcert CA install, `setcap`, datadir init, first-run detection, and GUI-driven site sync with privilege escalation. Tray "Start All" here assumes the stack is installed and sites already synced (via `laragonctl`/Plan 3b).
