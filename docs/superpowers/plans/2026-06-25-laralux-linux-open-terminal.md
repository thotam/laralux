...
- Run core tests with `cargo test -p laralux-core`; build `cargo build -p laralux-desktop`. If `cargo`/`node` aren't on PATH use `$HOME/.cargo/bin/cargo` / `$HOME/.nvm/versions/node/v24.16.0/bin/node`.
...
Run: `cargo test -p laralux-core terminal`
...
Run: `cargo test -p laralux-core terminal` then `cargo build -p laralux-core`
...
**Interfaces:**
- Consumes: `laralux_core::open_terminal`.
...
**3. Type consistency:**
- `open_terminal(&Path) -> Result<(), TerminalError>` (Task 1) called as `laralux_core::open_terminal(&dir)` in Task 2. ✓
