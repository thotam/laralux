...
**Tech Stack:** Rust (laralux-core, zero Tauri deps), serde_json, mkcert/nginx/Valkey static binaries.
...
- [ ] **Step 2: Run** — `cargo test -p laralux-core mkcert_static` → FAIL (some items missing) — actually it compiles the pure fns; ensure the test passes the URL assertion. If it already passes, proceed.
...
- [ ] **Step 2: Run** — `cargo test -p laralux-core nginx_static` → PASS for the pure fns (write impl if the module doesn't compile).
...
- [ ] **Step 4: Run + Commit** — `cargo test -p laralux-core` PASS; build clean.
...
- [ ] **Step 5: Run** — `cargo test -p laralux-core` PASS (update any `run_setup`/`detect`/`apt_packages_for` tests that assumed apt for these three); `cargo build -p laralux-desktop && cargo build -p laraluxctl` clean.
