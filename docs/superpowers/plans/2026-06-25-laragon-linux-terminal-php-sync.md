...
**Goal:** Make the shell `php` and `composer` use Laragon's active PHP version — install the CLI binary alongside php-fpm, expose the active version as `~/laragon/bin/php`, ship a `composer` wrapper that runs under it, and (opt-in) prepend `~/laragon/bin` to the user's shell PATH.

**Architecture:** Extend `php_static` to fetch the `cli` SAPI too; add `php_cli` (active `php` symlink + `ensure_active_php_cli` + composer.phar/wrapper) and `shell_env` (managed PATH block in rc files). A `Config.shell_integration` flag + Tauri toggle drive enable/disable. `set_php_version` re-points the symlink on switch. Also unify laragonctl's nginx resolution with the orchestrator's resolver.
...
- Active CLI is `~/laragon/bin/php` → symlink to `php<minor>`; binaries mode `0o755`; everything under user-owned `~/laragon` (no privilege).
- Shell PATH block markers `# >>> laragon >>>` / `# <<< laragon <<<`, body `export PATH="$HOME/laragon/bin:$PATH"` (literal `$HOME`). Modify existing `~/.bashrc`/`~/.zshrc`; if neither exists create the one matching `$SHELL` (zsh→`.zshrc` else `.bashrc`).
- composer wrapper `~/laragon/bin/composer` = `#!/bin/sh\nexec "$(dirname "$0")/php" "$(dirname "$0")/composer.phar" "$@"\n`, mode 0755; `composer.phar` from `https://getcomposer.org/composer.phar`.
...
- Run core tests with `cargo test -p laragon-core`; build `cargo build -p laragon-desktop && cargo build -p laragonctl`. If `cargo`/`node` aren't on PATH use `$HOME/.cargo/bin/cargo` / `$HOME/.nvm/versions/node/v24.16.0/bin/node`.
...
**Interfaces:**
- Consumes: `php_static::install_php_cli`, `Downloader`, `CommandRunner`, `LaragonPaths`, `PhpStaticError`.
- Produces:
...
Run: `cargo test -p laragon-core php_cli`
...
Run: `cargo test -p laragon-core shell_env`
...
git commit -m "feat(core): managed shell PATH block for ~/laragon/bin (shell_env)"
```
...
### Task 5: IPC + Setup + laragonctl wiring
...
- Modify: `laragonctl/src/main.rs` (unify nginx resolution)

**Interfaces:**
- Consumes: `ensure_active_php_cli`, `install_composer`, `enable_shell_path`, `disable_shell_path`, `set_active_php`, `bin::resolve_bin`.
- Produces: `terminal_integration_status()`, `set_terminal_integration(app, enabled)`; `set_php_version` re-points the symlink; setup creates the active symlink; laragonctl resolves nginx via `resolve_bin`.

- [ ] **Step 1: Imports in commands.rs**

In `src-tauri/src/commands.rs`, add to the `use laragon_core::{...}` block: `ensure_active_php_cli`, `install_composer`, `enable_shell_path`, `disable_shell_path`. (`CurlDownloader`, `RealCommandRunner`, `Config` are already imported.)
...
In `core/src/setup.rs`, in the `run_setup` PHP-static block, after persisting `cfg.php_version = ver` (on the `Some(ver)` arm), add the symlink so `~/laragon/bin/php` exists for the default version:
...
- [ ] **Step 6: Unify laragonctl nginx resolution**

In `laragonctl/src/main.rs`, in the `setup-perms` arm, replace:
...
```bash
git add src-tauri/src/commands.rs src-tauri/src/main.rs core/src/setup.rs laragonctl/src/main.rs
git commit -m "feat(desktop): terminal integration commands; symlink on switch/setup; unify laragonctl nginx resolve"
```
...
- §3.7 laragonctl nginx resolver unify → Task 5 Step 6. ✓
