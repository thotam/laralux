...
**Goal:** Source every PHP version from prebuilt static php-fpm binaries (dl.static-php.dev `bulk`) downloaded into `~/laralux/bin`, replacing the apt/ondrej path so PHP install/switch works on any distro with no root.

**Architecture:** New `core::php_static` module resolves a minor version to its newest patch from the `bulk` directory JSON, downloads + extracts the `php-fpm` binary into `~/laralux/bin/php-fpm<minor>` (via the existing `Downloader`/`CommandRunner` seams, no privilege). Detection/switch/UI from the prior slice are reused unchanged. The IPC install path and the Setup wizard switch to static; the apt/ondrej PHP code is removed.

**Tech Stack:** Rust (laralux-core, zero Tauri deps; add `serde_json`), Tauri 2, vanilla JS frontend.
...
- The static binary is installed as `~/laralux/bin/php-fpm<minor>` (e.g. `php-fpm8.4`), mode `0o755`. No pkexec for PHP install/switch.
- Run core tests with `cargo test -p laralux-core`; build with `cargo build -p laralux-desktop`. If `cargo`/`node` aren't on PATH use `$HOME/.cargo/bin/cargo` and `$HOME/.nvm/versions/node/v24.16.0/bin/node`.
...
**Interfaces:**
- Consumes: `LaraluxPaths`, `setup::Downloader`, `scaffold::CommandRunner`.
- Produces:
...
mod tests {
    ...
    fn install_php_static_downloads_extracts_and_places_binary() {
        ...
        assert!(bin.is_file(), "binary placed in ~/laralux/bin");
        #[cfg(unix)]
        {
...
/// `~/laralux/bin/php-fpm<version>` (mode 0755). No privilege required.
...
pub async fn install_php_version(
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<Vec<PhpVersionInfo>, String> {
        ...
        laralux_core::install_php_static(&state.paths, &version, &CurlDownloader, &RealCommandRunner)
...
- Modify: `laraluxctl/src/main.rs` (pass a runner)
...
In `laraluxctl/src/main.rs` (where `run_setup(&paths, &SudoPrivileged, &CurlDownloader)` is called), pass a runner:

```rust
            let report = run_setup(&paths, &SudoPrivileged, &CurlDownloader, &RealCommandRunner);
```

Add `RealCommandRunner` to laraluxctl's `use laralux_core::{...}` imports if not present.

- [ ] **Step 7: Run tests + build**

Run: `cargo test -p laralux-core setup` then `cargo build -p laralux-desktop && cargo build -p laraluxctl`
Expected: PASS — setup tests green; both binaries compile.

- [ ] **Step 8: Commit**

```bash
git add core/src/setup.rs core/src/php_versions.rs src-tauri/src/commands.rs laraluxctl/src/main.rs
...
- In the `tests` module, delete `apt_packages_are_laralux_parity`, `ondrej_suite_falls_back_to_newest_lts`, `install_php_adds_ondrej_installs_and_disables_unit`, and `install_php_surfaces_apt_error`. Keep `php_versions_marks_installed_and_active` and `php_versions_includes_unknown_installed` (both still valid for 8.0–8.5).
...
Run: `cargo run -p laralux-desktop`, open **Settings → PHP version**, click **Install** on a not-installed version (e.g. 8.4) → it downloads from dl.static-php.dev into `~/laralux/bin/php-fpm8.4` (no pkexec prompt) and the row becomes Installed. Click **Use** → toast "PHP 8.4 is now active"; if php-fpm was running it restarts on 8.4. (No JS test runner — human-verified.)
...
**3. Type consistency:**
- `install_php_static(&LaraluxPaths, &str, &dyn Downloader, &dyn CommandRunner) -> Result<(), PhpStaticError>` (Task 1) called by IPC (Task 2) and Setup (Task 3) with matching args. ✓
