...
**Tech Stack:** Rust (laragon-core, zero Tauri deps), MariaDB 11.4 LTS binary tarball, tar.
...
- `report.apt_packages` (a `Vec<String>`) MUST stay (laragonctl + frontend read it) — set it `Vec::new()`.
...
- [ ] **Step 2: Run** — `cargo test -p laragon-core mariadb_static` → PASS for the pure fns.
...
- [ ] **Step 1: Add basedir + update command/install_db_args.** In `core/src/service/mariadb.rs`:
  - Add: `fn basedir(&self, paths: &LaragonPaths) -> std::path::PathBuf { paths.bin().join("mariadb").join("current") }`.
  - `command`:
```rust
...
- [ ] **Step 3: Run + Commit** — `cargo test -p laragon-core mariadb` PASS; `cargo build -p laragon-desktop && cargo build -p laragonctl` clean.
...
```
(The tarball `mariadbd` lives under `~/laragon` — no Ubuntu AppArmor profile applies.) Remove the `run_setup_configures_mariadb_apparmor` test (it asserts the call happened). Keep `disable_system_services` + its test.

- [ ] **Step 6: Run + Commit** — `cargo test -p laragon-core` PASS (update any run_setup test that expected mariadb apt / apparmor); `cargo build -p laragon-desktop && cargo build -p laragonctl` clean.
...
- [ ] **Step 2: Remove from `core/src/privileged.rs`:**
  ...
  - The related unit tests: `fake_records_apt_installs`, `add_repo_argv_builds_add_apt_repository`, `fake_records_add_repo` (if present), `mariadb_apparmor_argv_runs_parser_with_laragon_rule`, the apparmor fake test.
...
**1. Spec coverage:** mariadb_static module (T1, §3.1); MariadbService basedir+init (T2, §3.2); run_setup wiring + apt empty + apparmor removed + report field (T3, §3.3); remove dead apt/apparmor machinery (T4, §3.4); lib re-exports (T1, §3.5). `report.apt_packages` kept empty (laragonctl/frontend). disable_system_services kept (best-effort). detect() already managed_bin_dirs (Spec 1).
