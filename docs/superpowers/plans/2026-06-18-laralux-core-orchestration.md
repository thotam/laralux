...
- Working directory layout root: `~/laralux/` (override-able by constructing `LaraluxPaths` with a custom root — tests use a tempdir).
- Config file: `~/laralux/laralux.toml` (TOML). Replaces Windows `laralux.ini`.
...
- Create: `laraluxctl/Cargo.toml`
- Create: `laraluxctl/src/main.rs`
- Create: `.gitignore`
...
- [ ] **Step 4: Create `laraluxctl/Cargo.toml`**
...
- [ ] **Step 5: Create `laraluxctl/src/main.rs`**

```rust
fn main() {
    println!("laraluxctl 0.1.0");
}
```
...
Run: `cargo test`
Expected: PASS — compiles both crates, `running 0 tests` for `laralux-core`.

- [ ] **Step 8: Commit**

```bash
git add Cargo.toml core laraluxctl .gitignore
git commit -m "chore: scaffold cargo workspace (core + laraluxctl)"
```

---

### Task 2: LaraluxPaths — directory layout
...
**Interfaces:**
- Produces:
  - `struct LaraluxPaths { root: PathBuf }`
  - `LaraluxPaths::new(root: PathBuf) -> LaraluxPaths`
  - `LaraluxPaths::default_root() -> PathBuf` (returns `$HOME/laralux`)
  - `fn root(&self) -> &Path`, `www`, `etc`, `data`, `log`, `tmp`, `ssl` → each `-> PathBuf`
  - `fn etc_for(&self, sub: &str) -> PathBuf` (e.g. `etc/nginx`)
  - `fn config_file(&self) -> PathBuf` (`<root>/laralux.toml`)
...
Run: `cargo test -p laralux-core paths`
Expected: FAIL — `cannot find type LaraluxPaths`.
...
git commit -m "feat(core): add LaraluxPaths directory layout"
```

---

### Task 3: Config — laralux.toml load/save
...
**Interfaces:**
- Consumes: `LaraluxPaths::config_file`
- Produces:
...
git commit -m "feat(core): add Config load/save (laralux.toml)"
```
...
- Produces:
  ...
  - `trait Service: Send + Sync` with:
    ...
    - `fn write_config(&self, paths: &LaraluxPaths) -> Result<(), ServiceError> { Ok(()) }`
    - `fn command(&self, paths: &LaraluxPaths) -> SpawnSpec;`
    - `fn health_check(&self, paths: &LaraluxPaths) -> Result<(), ServiceError>;`
    - `fn needs_init(&self, _paths: &LaraluxPaths) -> bool { false }`
    - `fn init(&self, _paths: &LaraluxPaths) -> Result<(), ServiceError> { Ok(()) }`
...
Run: `cargo test -p laralux-core service`
...
Run: `cargo test -p laralux-core process`
...
**Interfaces:**
- Consumes: `Service`, `ServiceKind`, `ServiceState`, `ServiceError`, `ProcessSpawner`, `Process`, `LaraluxPaths`
- Produces:
  - `struct Orchestrator`
  - `Orchestrator::new(paths: LaraluxPaths, services: Vec<Box<dyn Service>>, spawner: Box<dyn ProcessSpawner>) -> Orchestrator`
...
**Interfaces:**
- Consumes: `Service`, `ServiceKind`, `SpawnSpec`, `ServiceError`, `LaraluxPaths`
...
Run: `cargo test -p laralux-core redis`
...
Run: `cargo test -p laralux-core mailpit`
...
**Interfaces:**
- Consumes: `Service` types, `LaraluxPaths`
...
Run: `cargo test -p laralux-core mariadb`
...
**Interfaces:**
- Consumes: `PhpFpmService::socket_path` (to wire fastcgi), `Service` types, `LaraluxPaths`
...
Run: `cargo test -p laralux-core nginx`
...
**Interfaces:**
- Consumes: `Config`, all five service structs, `PhpFpmService::socket_path`, `LaraluxPaths`
- Produces: `fn build_services(config: &Config, paths: &LaraluxPaths) -> Vec<Box<dyn Service>>` — includes only services enabled in `config.services`; wires nginx's fastcgi socket to the php-fpm socket for `config.php_version`.
...
Run: `cargo test -p laralux-core registry`
...
### Task 14: `laraluxctl` CLI wiring + manual smoke test
...
**Interfaces:**
- Consumes: `LaraluxPaths`, `Config`, `build_services`, `Orchestrator`, `RealSpawner`, `ServiceKind`
...
Replace `laraluxctl/src/main.rs` with:

```rust
use laralux_core::{build_services, Config, LaraluxPaths, Orchestrator, RealSpawner, ServiceKind};
...
fn main() {
    ...
    match cmd.as_str() {
        ...
        _ => {
            println!("usage: laraluxctl <config-init|up|status>");
        }
    }
}
...
echo '<?php phpinfo();' > ~/laralux/www/index.php
cargo run -p laraluxctl -- up
# In another terminal:
...
```bash
git add laraluxctl/src/main.rs core/src/lib.rs
git commit -m "feat(laraluxctl): wire CLI up/down/status/config-init to core"
```
...
- `laralux.toml` config (spec §2, Global Constraints) → Task 3 ✓
...
- Working software exercising the stack → Task 14 `laraluxctl` ✓
...
**3. Type consistency:** `LaraluxPaths`, `Config`/`ServicesConfig`, `Service` trait method set, `SpawnSpec` builder (`new/arg/args/env/cwd`), `ProcessSpawner::spawn -> Box<dyn Process>`, `Process::{is_alive,stop,pid}`, `Orchestrator::{new,start,stop,state,refresh,start_order,start_all,stop_all}`, `build_services(&Config,&LaraluxPaths)`, `PhpFpmService::socket_path`, `ServiceKind` variants — all names/signatures used consistently across tasks. ✓
