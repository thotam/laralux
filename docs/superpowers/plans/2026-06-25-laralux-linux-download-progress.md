...
**Tech Stack:** Rust (laralux-core, zero Tauri deps), Tauri 2 (Emitter + events), vanilla JS, SVG/CSS.
...
- The CLI (`laraluxctl`) and all core tests pass `&NullProgress`. The desktop passes a `TauriProgress` only where a UI ring is wanted.
...
- Run core tests `cargo test -p laralux-core`; build `cargo build -p laralux-desktop && cargo build -p laraluxctl`; frontend `node --check dist/app.js` (node fallback `$HOME/.nvm/versions/node/v24.16.0/bin/node`). cargo fallback `$HOME/.cargo/bin/cargo`.
...
- [ ] **Step 2: Run** — `cargo test -p laralux-core progress` and `cargo test -p laralux-core parse_content_length` → FAIL.
...
**Files:** Modify `core/src/setup.rs`, `core/src/php_static.rs`, `core/src/php_cli.rs`, `core/src/coredns.rs`, `core/src/scaffold.rs`, `laraluxctl/src/main.rs`. (The `src-tauri` callers are updated in Task 3.)
...
This is an atomic signature change: all core call sites must be updated in the same commit so the crate compiles. Use `&crate::progress::NullProgress` at every test call site and in `laraluxctl`. **Do NOT change the `src-tauri` callers here** — they're in Task 3; this task must still leave `cargo build -p laralux-desktop` GREEN, which it does because the desktop calls these via the same names and Task 3 hasn't run — WAIT: changing a core signature breaks the desktop callers immediately. Therefore this task MUST also update the `src-tauri/src/commands.rs` call sites to pass `&crate::progress::NullProgress` (a temporary placeholder) so the workspace compiles; Task 3 swaps those placeholders for `&TauriProgress`. Update them to `&laralux_core::NullProgress` in this task.
...
- [ ] **Step 3: Update ALL core call sites + laraluxctl + (temporarily) the desktop callers** to pass a sink:
  ...
  - `laraluxctl/src/main.rs:93` `run_setup(&paths, &SudoPrivileged, &CurlDownloader, &RealCommandRunner)` → append `, &laralux_core::NullProgress` (add `NullProgress` to the `laralux_core::{...}` import).
  - `src-tauri/src/commands.rs` — TEMPORARILY append `&laralux_core::NullProgress` to: `run_setup(...)` (~line 155), `install_php_static(...)` (install_php_version ~line 403), `ensure_active_php_cli(...)` (set_php_version + the toggle path, ~lines 433/459), `ensure_coredns(...)` (apply_wildcard_dns ~line 527), `install_composer(...)` (~line 461), and `core_create_site(...)` (create_site command ~line 162 body). Add `NullProgress` to the `laralux_core::{...}` import. (Task 3 replaces these with `&TauriProgress` where a UI ring is wanted.)
...
- [ ] **Step 5: Run** — `cargo test -p laralux-core` → PASS; `cargo build -p laralux-desktop && cargo build -p laraluxctl` clean.

- [ ] **Step 6: Commit**

```bash
git add core/src/setup.rs core/src/php_static.rs core/src/php_cli.rs core/src/coredns.rs core/src/scaffold.rs laraluxctl/src/main.rs src-tauri/src/commands.rs
...
**Interfaces:** `struct TauriProgress(tauri::AppHandle)` impl `laralux_core::ProgressSink`. Replaces the `&laralux_core::NullProgress` placeholders from Task 2 with `&TauriProgress` in the commands that should show a ring.
...
- [ ] **Step 2: Wire the downloading commands** — in each, build `let progress = TauriProgress(app.clone());` (these commands already have `app: tauri::AppHandle` via `spawn_blocking(move || { let state = app.state...})` — clone `app` BEFORE the closure moves it, or use `app.clone()` where `app` is still available; if the command captured `app` into the closure, capture a clone for `TauriProgress`). Replace the Task-2 placeholder `&laralux_core::NullProgress` with `&progress` in:
  ...
  - Leave `ensure_coredns` (in `apply_wildcard_dns`) and the shell-toggle `ensure_active_php_cli`/`install_composer` calls on `&laralux_core::NullProgress` (no ring there for now — `apply_wildcard_dns` has only `&AppState`, no AppHandle; documented as deferred).
...
- [ ] **Step 3: Build** — `cargo build -p laralux-desktop` clean; `cargo build -p laraluxctl` clean (laraluxctl unchanged from Task 2).
