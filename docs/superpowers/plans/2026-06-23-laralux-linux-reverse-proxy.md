...
- Run core tests with `cargo test -p laralux-core`; build with `cargo build -p laralux-desktop`. If `cargo` is not on PATH, use `$HOME/.cargo/bin/cargo`. Syntax-check JS with `node --check dist/app.js` (if `node` is missing, use `$HOME/.nvm/versions/node/v24.16.0/bin/node`).
...
In `src-tauri/src/commands.rs`, update the first `use laralux_core::{...}` block to also import `ProxyRoute`:

```rust
use laralux_core::{
    build_services, create_site as core_create_site, detect_components, list_all_sites,
    run_setup, sync_sites, Config, CreateReport, LaraluxPaths, MkcertIssuer, Orchestrator,
